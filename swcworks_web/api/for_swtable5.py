...
def getAPIForSWTable5(request, *args, **kwargs):
    ...
    if user_group == 'admin':
        ...
        queryset = SWTable5.objects.filter(province = user_group)
...
@login_required
@csrf_exempt
def addAPIForSWTable5(request, *args, **kwargs):
    ret_data = {}
    print("[%s] ==== Try to add data to `SWTable5`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
    ...
    ### make obj attrs.
    attrs = {}
    if not post_data.get('num') or not re.search('^[0-9]+$',str(post_data['num'])):
        error_message = "ERROR: To add data failed. Field 'num' must be provided and should be a number."
        print(error_message)
        return HttpResponse(json.dumps({'message':error_message}), content_type='application/json', status=400)
    else:
        attrs['pxrc'] = int(post_data['num'])
...
@login_required
@csrf_exempt
def deleteAPIForSWTable5(request, *args, **kwargs):
    print("[%s] ==== Try to delete data from `SWTable5`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
    ...
    if 'id' not in post_data or not re.search('^[0-9]+$', str(post_data['id'])):
        ...
        print(error_message)
        return HttpResponse(json.dumps({'message':error_message}), content_type='application/json', status=400)
    queryset = SWTable5.objects.filter(id=post_data['id'])
    if queryset.exists():
    ...
    print("SUCCESS: data with id=%s deleted from `SWTable5`." % post_data['id'])
    return HttpResponse(json.dumps({}), content_type='application/json')


@login_required
@csrf_exempt
def updateAPIForSWTable5(request, *args, **kwargs):
    print("[%s] ==== Try to update data in `SWTable5`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
    ...
    if post_data.get('increase'):
        if re.search('^[0-9]+$',str(post_data['increase'])):
            obj.jqnzzl = int(post_data['increase'])
        else:
    ...
    print("SUCCESS: data with id=%s updated in `SWTable5`." % post_data['id'])
    return HttpResponse(json.dumps({}), content_type='application/json')
