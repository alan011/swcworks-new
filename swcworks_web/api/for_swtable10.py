...
def getAPIForSWTable10(request, *args, **kwargs):
    ...
    if user_group == 'admin':
        ...
        queryset = SWTable10.objects.filter(province = user_group)
    ...
    user_storage = os.path.join(config.FILE_STORAGE_ROOT, user_group, request.user.username, 'table10')
...
@login_required
@csrf_exempt
def addAPIForSWTable10(request, *args, **kwargs):
    ret_data = {}
    print("[%s] ==== Try to add data to `SWTable10`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
...
@login_required
@csrf_exempt
def deleteAPIForSWTable10(request, *args, **kwargs):
    print("[%s] ==== Try to delete data from `SWTable10`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
    ...
    if 'id' not in post_data or not re.search('^[0-9]+$', str(post_data['id'])):
        ...
        print(error_message)
        return HttpResponse(json.dumps({'message':error_message}), content_type='application/json', status=400)
    queryset = SWTable10.objects.filter(id=post_data['id'])
    if queryset.exists():
    ...
    print("SUCCESS: data with id=%s deleted from `SWTable10`." % post_data['id'])
    return HttpResponse(json.dumps({}), content_type='application/json')


@login_required
@csrf_exempt
def updateAPIForSWTable10(request, *args, **kwargs):
    print("[%s] ==== Try to update data in `SWTable10`:\nuser: %s" % (timezone.now().strftime('%d/%b/%Y %H:%M:%S'), request.user.username))
    ...
    print("SUCCESS: data with id=%s updated in `SWTable10`." % post_data['id'])
    return HttpResponse(json.dumps({}), content_type='application/json')
